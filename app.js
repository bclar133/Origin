...
function spinOffer() {
  ...
  if (Math.random() < 0.012) {
    const immortal = createPlayer(randomItem(DATA.immortalWildcards), { state: state.selectedState, year: team.year, immortal: true });
...
function selectOpponent(slotIndex) {
  ...
  if (Math.random() < 0.006) {
    const immortals = DATA.immortalWildcards
